...
colWinner b = foldr (\l w -> w || lineWinner l) False $ transpose b

-- | Return true if the board is a winner meaning there's a winning line
-- or column on the board
winner :: Board -> Bool
winner b = foldr (\l w -> w || lineWinner l) False b || colWinner b
...
solve2 puzzle =
  ...
  where
    ...
    go winners (play : remaining) nonWinners =
      -- plays and non-winning boards remain
